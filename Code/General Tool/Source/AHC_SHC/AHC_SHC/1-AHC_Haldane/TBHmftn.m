...
if 1==0
    [ftn58 , ainfo]=readwanniercorrect('wan_hr.dat',0);   
    save ftn58sp3.mat
else 
    load ftn58.mat
end
...
for ii=1:atn
    ...
    orbit(ii,1:length(space)) = str2num(buf((space(1)+1):end));
end
...
ftn58sparse.ij      = [ftn58(ib1,2:3);ftn58(ib2,2:3)];
ftn58sparse.tt      = [ftn58(ib1,4);ftn58(ib2,4)];
ftn58sparse.dd      = [ftn58(ib1,5:7);ftn58(ib2,5:7)];
end
...
% Add SOC %
if isSO==2
lamda_p = [0.1506 0.1506 2.1066 2.0705 0.4802 1.5232 0.7971 0.7921 0.5067];%*icase*0.2;
ipp=[1,5,9,13,17,21,25,29,33];
[ftn58, ftn58SO]=SOftn58sp3(1,ftn58,lamda_p,ipp);

ftn58sparse.isSO=1;
%[onsite, ib, newftn58]=getonsiteftn58(ftn58);
...
% ftn58 = spa2ftn(ftn58sparse);
% save ftn58soc.mat ftn58
